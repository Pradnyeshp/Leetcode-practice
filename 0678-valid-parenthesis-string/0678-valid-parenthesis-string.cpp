...
public:
    bool checkValidString(string s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            // If current character is an open bracket, push its index onto the
            // stack
            if (ch == '(') {
                openBrackets.push(i);
                // If current character is an asterisk, push its index onto the
                // stack
            } else if (ch == '*') {
                asterisks.push(i);
                // current character is a closing bracket ')'
            } else {
                // If there are open brackets available, use them to balance the
                // closing bracket
                if (!openBrackets.empty()) {
                    openBrackets.pop();
                    // If no open brackets are available, use an asterisk to
        ...
        // Check if there are remaining open brackets and asterisks that can
        // balance them
