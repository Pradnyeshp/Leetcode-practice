...
public:
    ...
    int depth(TreeNode* node) {
        ...
        int leftDepth =
            depth(node->left); // Recursively find depth of left subtree
        int rightDepth =
            depth(node->right); // Recursively find depth of right subtree

        // The path through this node = left depth + right depth
        diameter = max(diameter,
                       leftDepth + rightDepth); // Update max diameter if needed

        // Return the depth of the current subtree = 1 (current node) + max of
        // left/right
...
// for each node we left depth and right depth update the diameter as left +
// right recursively do for each node and keep track of maximium diameter.