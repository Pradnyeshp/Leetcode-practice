...
public:
    string minWindow(string s, string t) {
        ...
        int targetCharsRemaining =
            t.length(); // Number of characters we still need to find in the
                        // current window
        int minWindow[2] = {0, INT_MAX}; // Store the start and end indices of
        ...
        for (int endIndex = 0; endIndex < s.length(); endIndex++) {
            ...
            // If the character is needed (present in t and still has positive
            // count), decrease the target
            ...
            if (targetCharsRemaining == 0) {
                ...
                while (true) {
                    ...
                    // If removing s[startIndex] breaks the requirement, stop
                    // shrinking
                    ...
                    // Else, add the character back and move the left boundary
                    // forward
                    charCount[charAtStart]++;
                    startIndex++;
                }

                // Update the result if current window is smaller than the
                // previous best
                ...
                // Now move startIndex one step forward to look for the next
                // valid window
                charCount[s[startIndex]]++; // Since we are removing
                                            // s[startIndex] from the window
