...
public:
    ...
    // on the left or right child of the root. The length includes the root as
    // well.
    int solve(TreeNode* root, int parent) {
        ...
        // The number of nodes will be zero if the root value isn't equal to the
        // root. Otherwise return the max of left and right nodes plus one for
        // the root itself.
