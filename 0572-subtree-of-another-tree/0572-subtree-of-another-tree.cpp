...
public:
    bool isSubtree(TreeNode* root, TreeNode* subRoot) {
        ...
        // Hence, "tree rooted at node" cannot be equal to "tree rooted at
        // subRoot" "tree rooted at subRoot" will always be non-empty, as per
        // constraints
        if (root == nullptr) {
            return false;
        }

        // Check if the "tree rooted at root" is identical to "tree roooted at
        // subRoot"
        if (isIdentical(root, subRoot)) {
            return true;
        }

        // If not, check for "tree rooted at root.left" and "tree rooted at
        // root.right" If either of them returns true, return true
