...
public:
    int maxProfit(int k, vector<int>& prices) {
        ...
        for (;;) {
            ...
            for (v = p + 1;
                 (v + 1 < prices.size()) && (prices[v] >= prices[v + 1]); ++v)
                ;
            for (p = v; (p + 1 < prices.size()) && (prices[p] <= prices[p + 1]);
                 ++p)
                ;

            if (v ==
                p) { // v==p means that both v and p reach the end of the array
                break;
            }

            // Consider two transactions (v1, p1) (back of the stack) and (v2,
            // p2) (the new-found). If prices[v1] >= prices[v2], it is
            // meaningless to combine the two transactions. Save of profit of
            // (v1, p1), and pop it out of the record.
            while ((!vps.empty()) && (prices[v] <= prices[vps.top().first])) {
                profits.push_back(prices[vps.top().second] -
                                  prices[vps.top().first]);
                vps.pop();
            }
